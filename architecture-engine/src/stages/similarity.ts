...
export function computeSimilarity(
  ...
  for (const target of allFiles) {
    if (target.path === candidate.path) continue;

    let bestExportSimilarity = 0;
    for (const ce of candidate.exports) {
      for (const te of target.exports) {
        const sim = levenshteinSimilarity(ce, te);
        if (sim > bestExportSimilarity) bestExportSimilarity = sim;
      }
    }
    const exportScore = (bestExportSimilarity / 100) * 40;

    const propsJaccard = jaccardSimilarity(
      new Set(candidate.props),
      new Set(target.props)
    );
    const propsScore = propsJaccard * 30;

    const importJaccard = jaccardSimilarity(
      new Set(candidate.imports),
      new Set(target.imports)
    );
    const importScore = importJaccard * 10;

    const filenameSim = levenshteinSimilarity(candidate.basename, target.basename);
