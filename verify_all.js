...
files.forEach(f => {
  ...
  const hasRotated = (content.match(/rotate\(90,/g) || []).length;
  const hasRightX = (content.match(/右侧 X/g) || []).length;
  console.log(`${f.split('\\').pop()}: rotate90=${hasRotated}, rightXLabel=${hasRightX}`);
});
