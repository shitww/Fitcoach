...
// Fully deferred — only loaded after the user starts a workout session.
...
export default function WorkoutController() {
  ...
  // ── Workout Session Store (shadow layer for persistence / recovery) ──────────
  ...
  // ── Core session state (mirrored to shadow store for persistence) ──────────
  ...
  // Keep UI-level local states that are NOT part of core session
  ...
  // ── Phase 2: PR badge state ───────────────────────────────────────────────────
  const [prResult, setPrResult] = useState<{ type: 'weight' | 'reps' | 'volume'; display: string } | null>(null);
  ...
  const [completionWorkoutId, setCompletionWorkoutId] = useState<string | null>(null);
  ...
  // ── Shadow sync: mirror local state into zustand store ────────────────────────
  useEffect(() => {
    useWorkoutSession.setState({
      storedUserId: userId,
      exercises: exercises as any,
      activeExerciseName: currentExercise,
      savedExercises,
      customExercises,
      trainingNotes,
      lastExerciseRecord: lastExerciseRecord ?? null,
      weight,
      reps,
      rir,
      restTime,
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [exercises, currentExercise, savedExercises, customExercises, trainingNotes, lastExerciseRecord, weight, reps, rir, restTime, isBodyweight, userId]);

  // ── Session recovery from store (cross-tab / crash recovery) ──────────────────
  const [showRecoveryDialog, setShowRecoveryDialog] = useState(false);
  useEffect(() => {
    // Only check once on mount; if local exercises is empty but store has data,
    // offer recovery.
    if (exercises.length > 0 || !userId) return;
    const storeEx = useWorkoutSession.getState().exercises;
    if (storeEx.length > 0 && useWorkoutSession.getState().storedUserId === userId) {
  ...
  const logSet = () => {
    ...
    // PR detection (weight / reps / volume)
    const currentSets = exercises.find(e => e.name === currentExercise)?.sets ?? [];
    const allSets = [
      ...currentSets,
      ...(lastExerciseRecord ? [{ weight: lastExerciseRecord.weight, reps: lastExerciseRecord.reps }] : [])
    ];
    let detectedPR: { type: 'weight' | 'reps' | 'volume'; display: string } | null = null;
    if (allSets.length > 0 && !isBodyweight && !isCurrentExerciseTimed) {
      ...
      if (w > maxWeight) {
        detectedPR = { type: 'weight', display: `${w}kg` };
      } else if (vol > maxVolume) {
        detectedPR = { type: 'volume', display: `${vol}kg` };
      } else {
        const sameWeightSets = allSets.filter(s => s.weight === w);
        if (sameWeightSets.length > 0) {
          const maxReps = Math.max(...sameWeightSets.map(s => s.reps));
          if (r > maxReps) {
            detectedPR = { type: 'reps', display: `${r}次` };
          }
        }
      }
    }
    if (detectedPR) {
      startTransition(() => setPrResult(detectedPR));
      setTimeout(() => startTransition(() => setPrResult(null)), 3000);
    }
  ...
  const finishWorkout = async () => {
    ...
    try {
      ...
      try {
        ...
        // Also persist into store for cross-tab recovery
        useWorkoutSession.setState({
          pendingWorkout: {
            exercises: exercises as any,
            trainingNotes,
  ...
  return (
    <div className="min-h-screen px-4 py-5 sm:p-6 safe-bottom" style={{ background: 'var(--background)', color: 'var(--foreground)' }}>
      ...
      <div className="relative max-w-3xl mx-auto"
        ...
        <>
            ...
            {trainingType === 'strength' && (sessionPhase === 'active' || sessionPhase === 'paused') && (
              <div className="mb-4">
                <TrainingStatusBar
                  durationSec={useWorkoutTimer.getState().trainingDuration}
                  setCount={exercises.reduce((s, e) => s + e.sets.filter(st => !st.isWarmup && st.completed).length, 0)}
            ...
            {/* Training OS Intelligence Layer (Phase 4B) — lazy loaded */}
            {(sessionPhase === 'active' || sessionPhase === 'paused') && (
              <TrainingOSLayer
                intelligenceInput={{
                  exerciseHistories: [],
                  currentExercise: null,
                  userContext: { recentWorkouts: [], currentStreak: 0, daysSinceLastWorkout: 0 },
                  muscleMap: new Map(),
                  lastRecord: null,
                }}
              />
            ...
            {(trainingType === 'strength' || !trainingType) && (
              <>
                {/* Exercise History Strip — quick switch between recent exercises */}
                <ExerciseHistoryStrip
                  items={savedExercises.map(name => {
                  ...
                  currentExercise={currentExercise}
                  onSelect={guardedSelectExercise}
                />

                {/* Rest Timer Pill — lightweight floating rest indicator */}
                <RestTimerPill onSkip={storeSkipRest} exerciseName={currentExercise} />

                {/* Warmup suggestions (before first set) */}
                {currentExercise && exercises.find(e => e.name === currentExercise)?.sets.length === 0 && (
                  <WarmupCard
                    exerciseName={currentExercise}
                    onCompleteWarmup={(set) => {
                      // Warmup sets are not logged to the session store;
                      // they are informational only for now.
                      toast({ message: `热身完成 ${set.weight}kg × ${set.reps}`, type: 'info', duration: 1200 });
                    }}
                  />
                )}
                ...
                <ActiveExerciseCard
                  ...
                  completedSets={exercises.find(e => e.name === currentExercise)?.sets.map(s => ({ weight: s.weight, reps: s.reps, rir: s.rir, isBodyweight: s.isBodyweight })) ?? []}
                  ...
                  onRirChange={setRir}
                  onFocusWeight={() => {
                    setNumberPadTarget('weight');
                    setNumberPadValue(weight);
                    setNumberPadVisible(true);
                  }}
                  onFocusReps={() => {
                    setNumberPadTarget('reps');
                    setNumberPadValue(reps);
                    setNumberPadVisible(true);
                  }}
                      ...
                      <div key={ex.id} className="space-y-1.5">
                        ...
                        <div className="flex items-center gap-3 px-1">
                          ...
                          <div className="flex-1 min-w-0">
                            <span className="text-sm font-semibold truncate" style={{ color: 'var(--text-med)' }}>
                              {exBaseName}
                            </span>
                          </div>
            ...
            {/* PR Archive */}
            {trainingType === 'strength' && (() => {
              const prRecords: { date: string; type: 'weight' | 'reps' | 'volume'; value: string; exercise: string }[] = [];
              for (const ex of exercises) {
                const sets = ex.sets.filter(s => !s.isWarmup && s.completed);
                if (sets.length === 0) continue;
                const maxW = Math.max(...sets.map(s => s.weight));
                const maxV = Math.max(...sets.map(s => s.weight * s.reps));
                const maxReps = Math.max(...sets.map(s => s.reps));
                for (const s of sets) {
                  if (s.weight === maxW) {
                    prRecords.push({ date: new Date().toISOString().slice(0, 10), type: 'weight', value: `${s.weight}kg`, exercise: ex.name });
                    break;
                  }
                }
                for (const s of sets) {
                  if (s.reps === maxReps && s.weight < maxW) {
                    prRecords.push({ date: new Date().toISOString().slice(0, 10), type: 'reps', value: `${s.reps}次`, exercise: ex.name });
                    break;
                  }
                }
                for (const s of sets) {
                  if (s.weight * s.reps === maxV && s.weight < maxW) {
                    prRecords.push({ date: new Date().toISOString().slice(0, 10), type: 'volume', value: `${s.weight * s.reps}kg`, exercise: ex.name });
                    break;
                  }
                }
              }
              if (prRecords.length === 0) return null;
              return (
                <div className="mt-4">
                  <PRArchive records={prRecords} />
                </div>
              );
            })()}
      ...
      <SessionRecoveryDialog
        ...
        exerciseCount={ws.exercises.filter(e => e.sets.length > 0).length}
        setCount={ws.exercises.reduce((s, e) => s + e.sets.length, 0)}
        durationSec={useWorkoutTimer.getState().trainingDuration}
        onResume={() => {
          setShowRecoveryDialog(false);
          const snap = useWorkoutSession.getState();
          if (snap.exercises.length > 0) {
            setExercises(snap.exercises as Exercise[]);
            setCurrentExercise(snap.activeExerciseName);
            setSavedExercises(snap.savedExercises);
            setCustomExercises(snap.customExercises);
            setTrainingNotes(snap.trainingNotes);
            setWeight(snap.weight);
            setReps(snap.reps);
            setRir(snap.rir);
            setRestTime(snap.restTime);
            setIsBodyweight(snap.isBodyweight);
          }
        }}
        onDiscard={() => {
          setShowRecoveryDialog(false);
          useWorkoutSession.getState().clearSession();
        }}
      />
      ...
      <WorkoutCompletionCard
        ...
        prCount={(() => {
          ...
          for (const ex of exercises) {
            ...
            const maxV = Math.max(...sets.map(s => s.weight * s.reps));
            for (const s of sets) {
              if (s.weight === maxW || s.weight * s.reps === maxV) { c++; break; }
            }
          }
          return c;
        })()}
        exerciseNames={exercises.filter(e => e.sets.some(s => !s.isWarmup && s.completed)).map(e => e.name)}
        onDismiss={() => {
          setShowCompletionCard(false);
          if (completionWorkoutId) {
            router.push(`/summary?id=${completionWorkoutId}`);
          } else {
            router.push('/');
          }
        }}
      ...
      {/* NumberPad overlay for mobile weight/reps input */}
      <NumberPad
        ...
        onConfirm={() => {
          if (numberPadTarget === 'weight') {
            setWeight(numberPadValue);
          } else if (numberPadTarget === 'reps') {
            setReps(numberPadValue);
          }
          setNumberPadVisible(false);
        }}
        label={numberPadTarget === 'weight' ? '重量 (kg)' : numberPadTarget === 'reps' ? '次数' : undefined}
        allowDecimal={numberPadTarget === 'weight'}
      />
    </div>
  );
}
